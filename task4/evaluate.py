...
def checkpoint(path, device):
    state=torch.load(path, map_location=device, weights_only=False); model=CifarResNet18().to(device); model.load_state_dict(state['model']); return model
...
def row(method, score, known_validation, known_test, unknown, group):
    ...
    return {"method":method,"score":score,"unknown_group":group,"auroc":roc_auc_score(labels,values),"threshold":threshold,"known_test_acceptance_rate":float((known_test<=threshold).mean()),"unknown_rejection_rate":float((unknown>threshold).mean()),"fpr_at_95_tpr":float((unknown<=threshold).mean())}
...
def main():
    ...
    for specification in a.checkpoint:
        ...
        scores=('mls',) if method=='gcsc' else ('msp','mls','energy','mahalanobis')
        for group,indices in groups.items():
            ...
            for name in scores: rows.append(row(method,name,score_values(name,valid_z,valid_f,means,variance),score_values(name,test_z,test_f,means,variance),score_values(name,uz,uf,means,variance),group))
        for name in scores:
            near=np.load(cache/f'{method}_near_outputs.npz'); far=np.load(cache/f'{method}_far_outputs.npz'); rows.append(row(method,name,score_values(name,valid_z,valid_f,means,variance),score_values(name,test_z,test_f,means,variance),np.r_[score_values(name,near['logits'],near['features'],means,variance),score_values(name,far['logits'],far['features'],means,variance)],'all'))
